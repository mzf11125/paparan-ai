...
Key functions:
- tavily_search: General web search with source tier classification
- search_government_sources: Search official government/institutional domains only
- search_policy_research: Search think tanks and research institutes
- search_asean_news: Recent ASEAN/Southeast Asia news
- search_financial_intelligence: Financial and economic data sources
- scrape_source_tier: Deep-search specific trusted source domain
- fetch_webpage: Direct URL content extraction with markdown conversion
"""
...
def deduplicate_results(results: list[SearchResult]) -> list[SearchResult]:
    """
    Deduplicate search results by URL and similar titles.

    Args:
        results: List of SearchResult objects.

    Returns:
        Deduplicated list with highest-scoring items kept.
    """
    seen_urls: set[str] = set()
    seen_titles_normalized: dict[str, SearchResult] = {}
    ...
    for result in results:
        ...
        # Normalize title for similarity checking
        title_norm = result.title.lower().strip()
        title_words = set(w for w in title_norm.split() if len(w) > 3)

        # Check for similar titles (keep higher priority_weight)
        is_duplicate = False
        for existing_norm, existing_result in list(seen_titles_normalized.items()):
            existing_words = set(existing_norm.split())
            overlap = len(title_words & existing_words)
            if overlap > 0 and overlap / min(len(title_words), len(existing_words)) > 0.6:
                # Similar title - keep higher priority
                if result.priority_weight >= existing_result.priority_weight:
                    del seen_titles_normalized[existing_norm]
                else:
                    is_duplicate = True
                    break

        if not is_duplicate:
            seen_titles_normalized[" ".join(title_words)] = result
            deduped.append(result)

    return deduped


def filter_by_recency(results: list[SearchResult], max_days: int) -> list[SearchResult]:
    """
    ...
    for result in results:
        ...
        try:
            # Parse date (handles ISO format and common date strings)
...
def rank_by_relevance(
    ...
    query: str,
    boost_tier: bool = True,
) -> list[SearchResult]:
    """
    Re-rank results by relevance score combining tier, Tavily score, and query match.

    Args:
        results: List of SearchResult objects.
        query: Original search query for relevance matching.
        boost_tier: Whether to boost government/official sources.
    ...
    def compute_score(result: SearchResult) -> float:
        score = 0.0

        # Priority weight (0.5 - 1.0)
        if boost_tier:
            score += result.priority_weight * 0.4

        # Tavily score if available
        if result.tavily_score:
            score += result.tavily_score * 0.3
...
def batch_search(
    ...
    """
    Run multiple searches in batch and aggregate/deduplicate results.

    Args:
        queries: List of search queries to execute.
        topic: Tavily topic filter.
        max_results_per_query: Results to fetch per query.
        max_total_results: Maximum results to return after deduplication.

    Returns:
        SearchBundle with aggregated results.
    """
    ...
    for query in queries:
        ...
        total_api_calls += bundle.api_calls
    ...
    # Rank and limit
    ranked = rank_by_relevance(deduped, " ".join(queries))
    final_results = ranked[:max_total_results]
    ...
    return SearchBundle(
        ...
        elapsed_ms=0.0,  # Could track if needed
    )
